...
async function createTables() {
  ...
  const tables = [
    ...
    // Provider Portal — Provider Users
    ...
    // Provider Portal — Provider Roles
    ...
    // Provider Portal — Payments (Billing)
    ...
    // Provider Portal — Support Tickets (CRM)
...
// ============================================================================
// 9. CREATE PROVIDER TABLES & SEED DATA
// ============================================================================

async function createProviderTables() {
  console.log('🛡️  Step 9: Setting up Provider Portal tables...\n');

  // Seed default provider roles
  const providerRoles = [
    { role_name: 'PROVIDER_OWNER', label: 'Owner', description: 'Full system access', permissions: JSON.stringify(['dashboard','organizations','users','billing','crm','monitoring','staff','roles','setup']), is_system: true },
    { role_name: 'PROVIDER_ADMIN', label: 'Admin', description: 'Manage orgs, users, staff, billing, CRM', permissions: JSON.stringify(['dashboard','organizations','users','billing','crm','monitoring','staff']), is_system: true },
    { role_name: 'PROVIDER_BILLING', label: 'Billing Manager', description: 'Billing and payments only', permissions: JSON.stringify(['dashboard','billing']), is_system: true },
    { role_name: 'PROVIDER_SUPPORT', label: 'Support Agent', description: 'CRM and support tickets only', permissions: JSON.stringify(['dashboard','crm']), is_system: true },
    { role_name: 'PROVIDER_MONITOR', label: 'Monitor', description: 'Read-only dashboard and monitoring', permissions: JSON.stringify(['dashboard','monitoring']), is_system: true },
  ];

  const { error: roleError } = await supabase
    .from('provider_roles')
    .upsert(providerRoles, { onConflict: 'role_name' });

  if (roleError) {
    console.error('  ✗ Failed to seed provider roles:', roleError.message, '\n');
  } else {
    console.log(`  ✓ Seeded ${providerRoles.length} provider roles\n`);
  }

  // Seed default provider user (pin: provider123)
  ...
  if (userError) {
    console.error('  ✗ Failed to seed default provider user:', userError.message, '\n');
  } else {
    console.log('  ✓ Default provider user created (provider / provider123)\n');
  }
}

// ============================================================================
// 10. CREATE ADMIN USER INSTRUCTIONS
// ============================================================================

function showAdminUserInstructions() {
  console.log('👤 Step 10: Creating Admin User\n');
...
async function main() {
  ...
  // Create provider portal tables and seed data
  await createProviderTables();
  ...
  console.log('  ✓ 5 Provider Roles');
  console.log('  ✓ 1 Default Provider User (provider / provider123)\n');
  console.log('📌 Provider Portal Login: username: provider | PIN: provider123');
  console.log('📌 Client Portal Login: email: admin@biobridge.com | password: Admin@123\n');
