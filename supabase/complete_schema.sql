...
CREATE POLICY org_isolation_attendance_logs ON attendance_logs
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));

CREATE POLICY org_isolation_attendance_daily ON attendance_daily
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));

CREATE POLICY org_isolation_leave_types ON leave_types
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));

CREATE POLICY org_isolation_leave_balances ON leave_balances
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));
...
CREATE POLICY org_isolation_salary_components ON salary_components
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));

CREATE POLICY org_isolation_payroll_runs ON payroll_runs
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));

CREATE POLICY org_isolation_payroll_records ON payroll_records
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));

CREATE POLICY org_isolation_items ON items
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));

CREATE POLICY org_isolation_stock ON stock
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));
...
CREATE POLICY org_isolation_crm_contacts ON crm_contacts
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));

CREATE POLICY org_isolation_crm_opportunities ON crm_opportunities
    FOR ALL
    USING (organization_id IN (SELECT organization_id FROM users WHERE auth.uid() = users.auth_id));
